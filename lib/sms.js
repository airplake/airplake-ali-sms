...
AliyunSms.prototype = {
  ...
  _sendMessage: function (data) {
    ...
    return new Promise((resolve, reject) => {
      request.post({
       ...
       //  console.log('_sendMessage',body)
        if (response.statusCode === 201 || response.statusCode === 200) {
          resolve(JSON.parse(body))
        } else {
          // console.log('_sendMessage',err)
  ...
  _signParameters: function (param, AccessKeySecret) {
    ...
    // 发送注册短信[业务级]
  sendRegistSms: function (obj) {
    // var data = {
    //   PhoneNumbers: '', // 要发送到短信的手机
    //   SignName: '', // 短信签名，阿里云短信平台申请
    //   TemplateCode: '', // 短信模板Code，阿里云短信平台申请
    //   TemplateParam: '{"name":""}', // 短信模板中参数指定，以你的为准替换之
    //   OutId: '1234'// 可选
    // }
